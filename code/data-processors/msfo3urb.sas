...
data enrollee_o3; 
	...
	where sex ne 'U' and pm_1yr ne . and o3max8h_warm ne . and ses_zip ne . and ruca ne '';
run;
...
*Count the number of enrollees of age a, sex s and race r by zipcode z at the beginning of the month t ;
	...
	select zip_code, year, month, enrollee_age, sex, race, o3max8h_warm, pm_1yr, state, ruca, ses_zip, ses_stt, count(distinct BENE_ID) as no_enrollee
	from enrollee_o3
	group by zip_code, year, month, enrollee_age, sex, race, o3max8h_warm, pm_1yr, state, ruca, ses_zip, ses_stt;
...
data master_cuz; 
	set master_cuz;
	if ruca="Urban" then urb=0; else urb=1;
	if ruca="Micropolitan" then micro=0; else micro=1;
	if ruca="Rural" then rur=0; else rur=1;
	o3urb=o3max8h_warm*urb;
	o3micro=o3max8h_warm*micro;
	o3rur=o3max8h_warm*rur;
	drop ruca;
run;
...
proc export 
	data=master_cuz
	outfile='/scratch/fatemehkp/projects/Zipcode Ozone PM/data/analysis/ndi-o3-zipcd-urban.csv'  
	dbms=csv replace;
run;
