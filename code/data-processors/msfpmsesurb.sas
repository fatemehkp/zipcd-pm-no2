...
data enrollee_pm; 
	...
	where sex ne 'U' and pm_1yr ne . and ses_zip ne . and ses_tertile ne .;
run;
...
proc sql;
	create table master_cuz_sescat as
	select *
	from master_cuz a
	inner join ses.ses_zipcd_urban_cat b
	on a.zip_code=b.zip_code and a.year=b.year;
quit;

data master_cuz_sescat; 
	set master_cuz_sescat;
...
proc export 
	data=master_cuz_sescat
	outfile='/scratch/fatemehkp/projects/Zipcode PM NO2/data/analysis/ndi-pm-zipcd-urban-sescat.csv'  
	dbms=csv replace;
run;
