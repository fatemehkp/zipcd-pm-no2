...
data enrollee_no2; 
	set cms.enrollee65_ndi_0008_clean_rc8;
...
proc sort data=master_cuz_ses nodupkey out=pmno2;
	by zip_code year month;
run;

ods graphics off;
proc reg data=pmno2;
      model PM_1yr=NO2_1yr;
      output out=PM_resi
         r=PM_NO2;
		 label PM_NO2=PM_NO2;
run;
...
ods graphics off;
proc reg data=PM_resi;
      model NO2_1yr=PM_1yr;
      output out=NO2_resi
         r=NO2_PM;
		 label NO2_PM=NO2_PM;
run;
...
proc export 
	data=master_cuz_ses
	outfile='/scratch/fatemehkp/projects/Zipcode PM NO2/data/analysis/ndi-no2-zipcd-rc8.csv'  
	dbms=csv replace;
run;
