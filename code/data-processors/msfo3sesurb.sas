/* Created by Fatemeh K (07-28-2022) (08-19-2022)																*/
...
/* Interaction term for ses category in urban areas																*/
...
data enrollee_o3; 
	...
	where sex ne 'U' and pm_1yr ne . and o3max8h_warm ne . and ses_zip ne . and ses_tertile ne .;
run;
...
*Count the number of enrollees of age a, sex s and race r by zipcode z at the beginning of the month t ;
	...
	select zip_code, year, month, enrollee_age, sex, race, o3max8h_warm, pm_1yr, state, ses_zip, ses_stt, ses_tertile, count(distinct BENE_ID) as no_enrollee
	from enrollee_o3
	group by zip_code, year, month, enrollee_age, sex, race, o3max8h_warm, pm_1yr, state, ses_zip, ses_stt, ses_tertile;
...
data master_cuz; 
	set master_cuz;
	if ses_tertile=0 then sesL=0; else sesL=1;
	if ses_tertile=1 then sesM=0; else sesM=1;
	if ses_tertile=2 then sesH=0; else sesH=1;
	o3sesL=o3max8h_warm*sesL;
	o3sesM=o3max8h_warm*sesM;
	o3sesH=o3max8h_warm*sesH;
	drop ses_tertile;
run;
...
data master_cuz; 
	...
	StrID= agec || sex || race;
	drop agec enrollee_age sex race;
run;


proc export 
	data=master_cuz
	outfile='/scratch/fatemehkp/projects/Zipcode Ozone PM/data/analysis/ndi-o3-zipcd-urban-sescat.csv'  
	dbms=csv replace;
run;
